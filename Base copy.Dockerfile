...
ARG ARCH
FROM --platform=${ARCH} ubuntu:25.04
...
#     fi; cat /etc/apt/sources.list;
# RUN echo "Acquire::https::${REPOGITORY_URL}::Verify-Peer \"false\";" > /etc/apt/apt.conf.d/99verify-peer.conf
...
#        --with-libxml-dir \
    ...
    ARCH=$(uname -m); \
    case $ARCH in \
    "x86_64")   DOCKERIZE_FILE=dockerize-linux-amd64 ;; \
    "aarch64")  DOCKERIZE_FILE=dockerize-linux-arm64 ;; \
    "ppc64le")  DOCKERIZE_FILE=dockerize-linux-ppc64le ;; \
    *) echo "Unsupported architecture: $ARCH"; exit 1 ;; \
    esac; \
    wget-retry https://github.com/jwilder/dockerize/releases/download/v${DOCKERIZE_VERSION}/${DOCKERIZE_FILE}-v${DOCKERIZE_VERSION}.tar.gz; \
    tar -C /usr/local/bin -xzvf ${DOCKERIZE_FILE}-v${DOCKERIZE_VERSION}.tar.gz; \
    rm -rf ${DOCKERIZE_FILE}-v${DOCKERIZE_VERSION}.tar.gz; \
    \
    # Install supervisor
